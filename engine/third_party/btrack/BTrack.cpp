...
//=======================================================================
BTrack::BTrack (int hop, int frame, int sampleRate)
 : odf (hop, frame, ComplexSpectralDifferenceHWR, HanningWindow)
{
...
{	
	...
	beatPeriod = round (60 / ((((double) hopSize) / samplingFrequency) * 120.));
...
{
    // Lazerdeck: linear-interpolation resample of the onset detection function
    // to a fixed length of 512 samples. Replaces the upstream libsamplerate
    // (src_simple) dependency. For hopSize 512, onsetDFBufferSize == 512 and
    // this reduces to an exact copy.
    const int outputLength = 512;
    const int inputLength = onsetDFBufferSize;
    ...
    for (int i = 0; i < outputLength; i++)
    {
        double pos = (inputLength > 1)
            ? ((double) i * (double) (inputLength - 1)) / (double) (outputLength - 1)
            : 0.0;
        int idx = (int) pos;
        double frac = pos - (double) idx;
        double a = onsetDF[idx];
        double b = (idx + 1 < inputLength) ? onsetDF[idx + 1] : a;
        resampledOnsetDF[i] = a + frac * (b - a);
    }
}
...
void BTrack::calculateTempo()
{
	...
	beatPeriod = round ((60.0 * (double) samplingFrequency) / (((2 * maxIndex) + 80) * ((double) hopSize)));

	if (beatPeriod > 0)
        estimatedTempo = 60.0 / ((((double) hopSize) / (double) samplingFrequency) * beatPeriod);
}
...
void BTrack::updateCumulativeScore (double onsetDetectionFunctionSample)
{
	...
	std::vector<double> logGaussianTransitionWeighting (windowSize);
...
{	 
	...
	std::vector<double> futureCumulativeScore (onsetDFBufferSize + beatExpectationWindowSize);
	std::vector<double> beatExpectationWindow (beatExpectationWindowSize);
	...
	std::vector<double> logGaussianTransitionWeighting (pastWindowSize);
	...
	for (int i = onsetDFBufferSize; i < (onsetDFBufferSize + beatExpectationWindowSize); i++)
	{
        ...
        futureCumulativeScore[i] = calculateNewCumulativeScoreValue (futureCumulativeScore.data(), logGaussianTransitionWeighting.data(), startIndex, endIndex, 0.0, 1.0);
