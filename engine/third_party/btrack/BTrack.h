...
public:
    ...
    /** Constructor taking hopSize, frameSize and the audio sampling frequency.
     * Lazerdeck addition: the upstream algorithm hardcodes 44100 Hz in its
     * tempo conversions; this lets the engine pass the real buffer rate so BPM
     * estimates stay correct at 48 kHz etc.
     * @param hopSize the hop size in audio samples
     * @param frameSize the frame size in audio samples
     * @param sampleRate the audio sampling frequency in Hz
     */
    BTrack (int hopSize, int frameSize, int sampleRate);
...
private:
    ...
    int samplingFrequency = 44100;          /**< the audio sampling frequency in Hz (Lazerdeck: replaces hardcoded 44100) */
